...
///
/// - static const <type> <name>_default
/// - void reset_<name>()
/// - void store_<name>()
/// - void restore_<name>()
/// - void set_<name>(<type>)
...
#define ALBERT_PLUGIN_PROPERTY_BASE(type, name, defaultValue) \
    ...
    protected: void restore_##name(std::unique_ptr<QSettings> &s) { set_##name##_(s->value(EXPAND_STRINGIZE(name), name##_default).value<type>()); } \
    ...
    public: void set_##name(type val) { if (val != name()){ set_##name##_(val); store_##name(); emit name##_changed(); } }
...
#define ALBERT_PLUGIN_PROPERTY_MEMBER(type, name, member, defaultValue) \
    ALBERT_PLUGIN_PROPERTY_BASE(type, name, defaultValue) \
...
#define ALBERT_PLUGIN_PROPERTY(type, name, defaultValue) \
    ALBERT_PLUGIN_PROPERTY_MEMBER(type, name, name##_, defaultValue) \
...
/// @param widget The widget pointer to connect to
///
#define ALBERT_PLUGIN_PROPERTY_CONNECT_CHECKBOX(object, name, widget) \
    ALBERT_PLUGIN_PROPERTY_CONNECT(object, name, widget, setChecked, toggled)
