...
std::map<std::pair<QString,QString>,float> GlobalQueryHandlerPrivate::usage_scores;
...
void GlobalQueryHandlerPrivate::setScores(std::map<std::pair<QString, QString>, float> scores)
{
    unique_lock lock(m);
...
void GlobalQueryHandlerPrivate::applyUsageScores(vector<RankItem> &rank_items) const
{
    ...
    for (auto & rank_item : rank_items){
        ...
        if (prio_perfect_match && rank_item.score == 1.0f){  // Prefer exact matches
            rank_item.score = 2.0f/3;
            try {
                rank_item.score += usage_scores.at(make_pair(q->id(), rank_item.item->id()))/3;
            } catch (const out_of_range &){}
        } else {
            try {
                rank_item.score = usage_scores.at(make_pair(q->id(), rank_item.item->id()))/3 + 1.0f/3;
            } catch (const out_of_range &){
